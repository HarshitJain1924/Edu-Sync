...
import { Brain, ArrowLeft, Users, TrendingUp } from "lucide-react";
...
export default function AdminLearningStyles() {
  ...
  const fetchStudentLearningStyles = async () => {
    try {
      ...
      const formattedData: StudentLearningStyle[] = (data || []).map((item: any) => ({
        ...
        auditory_score: item.auditory_score,
        kinesthetic_score: item.kinesthetic_score,
        reading_writing_score: item.reading_writing_score,
  ...
  const stylePercentages = {
    visual: totalStudents > 0 ? Math.round((styleCounts.visual / totalStudents) * 100) : 0,
    auditory: totalStudents > 0 ? Math.round((styleCounts.auditory / totalStudents) * 100) : 0,
    kinesthetic: totalStudents > 0 ? Math.round((styleCounts.kinesthetic / totalStudents) * 100) : 0,
    reading_writing: totalStudents > 0 ? Math.round((styleCounts.reading_writing / totalStudents) * 100) : 0
  };
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2">Learning Styles Analytics</h1>
          <p className="text-muted-foreground">Platform-wide student learning preferences and insights</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card className="border-blue-500/20 bg-blue-500/5">
            <CardContent className="p-4">
              ...
              <Progress value={stylePercentages.visual} className="h-2" />
              <p className="text-xs text-muted-foreground mt-1">{stylePercentages.visual}% of students</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-purple-500/20 bg-purple-500/5">
            <CardContent className="p-4">
              ...
              <Progress value={stylePercentages.auditory} className="h-2" />
              <p className="text-xs text-muted-foreground mt-1">{stylePercentages.auditory}% of students</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-green-500/20 bg-green-500/5">
            <CardContent className="p-4">
              ...
              <Progress value={stylePercentages.kinesthetic} className="h-2" />
              <p className="text-xs text-muted-foreground mt-1">{stylePercentages.kinesthetic}% of students</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-orange-500/20 bg-orange-500/5">
            <CardContent className="p-4">
              ...
              <Progress value={stylePercentages.reading_writing} className="h-2" />
              <p className="text-xs text-muted-foreground mt-1">{stylePercentages.reading_writing}% of students</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-muted rounded-lg">
                <h4 className="font-semibold mb-2">Total Assessments</h4>
                <p className="text-3xl font-bold text-primary">{totalStudents}</p>
                <p className="text-sm text-muted-foreground">Students completed the quiz</p>
              </div>
              <div className="p-4 bg-muted rounded-lg">
                <h4 className="font-semibold mb-2">Most Common Style</h4>
                <p className="text-2xl font-bold text-primary">
                  {Object.entries(styleCounts).sort(([,a], [,b]) => b - a)[0]?.[0] 
                    ? getStyleLabel(Object.entries(styleCounts).sort(([,a], [,b]) => b - a)[0][0])
                    : "N/A"}
                </p>
                <p className="text-sm text-muted-foreground">
                  {Math.max(...Object.values(styleCounts))} students
                </p>
              </div>
            </div>
          </CardContent>
